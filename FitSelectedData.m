function FitSelectedData(hObject,handles,SelInd)
% Fit the data in row SelInd of results table


FixOmega = get(handles.FitOmegaCheck,'Value');
FixGamma = get(handles.FitGammaCheck,'Value');
FixNoiseFloor = get(handles.FitNoiseFloorCheck,'Value');
FixAmp = get(handles.FitAmpCheck,'Value');
FitParams = [FixOmega,FixGamma,FixNoiseFloor,FixAmp];
...
switch model
    ...
    case 'Mechanical susceptibility'
        [fit,gof,out]=MechSuscFit(Freq(valid),Amp(valid),StartPoint,FitParams);
    case 'Squashing model'
...
if handles.DoOptimization
    ...
    for i = 1:NumIter
        ...
        if ~FixOmega
            p4 = random('norm',1,Sigma,[1,1]);
            StartPoint(4) = StartPoint0(4)*p4;
        end
        switch model
            case 'Lorentzian'
                [thisfit,thisgof, out]=LogLorentzianFit(Freq(valid),Amp(valid),StartPoint);
            case 'Mechanical susceptibility'
                [thisfit,thisgof,out]=MechSuscFit(Freq(valid),Amp(valid),StartPoint,FitParams);
            case 'Squashing model'
