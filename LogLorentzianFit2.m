function [fitresult, gof, output] = LogLorentzianFit2(Freq, Amp, StartPoint)
...
ft = fittype( 'log(c+a/(1+((x-x0)/(L/2))^2))', 'coefficients', {'L','a'},'independent', 'x', 'dependent', 'y','problem',{'c','x0'} );
opts = fitoptions( ft);
...
opts.StartPoint = StartPoint(1:2);



% Fit model to data.
[fitresult, gof, output] = fit( xData, log(yData), ft, opts, 'problem',{StartPoint(3),StartPoint(4)});
% Plot fit with data.
